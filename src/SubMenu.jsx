...
function SubMenu({ isOpen, showLinkAnimation, rotes, setIsOpen }) {
    ...
    return (<>
        <div className="rotes subRotes" >
            <div className="rotesItem" onClick={() => {
                ...
                <AnimatePresence>
                    {isOpen && <motion.div
                        ...
                        animate="show"
                        exit="hidden"
                        className='rotesText'>{rotes.name} </motion.div>}
                </AnimatePresence>
                {isOpen && <motion.div className='rotesIcon' animate={isMenuOpen ? { rotate: -90 } : { rotate: 0 }}><FaAngleDown size={20}/></motion.div>}
            </div>
            <AnimatePresence >
